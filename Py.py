...
def process_webhook():
    try:
        ...
        logger.info(f"📩 Incoming update: {json_str[:400]}")  # log isi update
...
def ai_chat(message):
    try:
        ...
        if len(command_parts) <= 1:
            bot.reply_to(message, "❌ Harap masukkan pertanyaan.\nContoh:\n/ai apa itu python?")
            return
...
def facebook_download(message):
    try:
        command_parts = message.text.split(' ', 1)
        if len(command_parts) <= 1:
            bot.reply_to(message, "Usage: /facebook <link>")
            return
...
def echo_message(message):
    command_parts = message.text.split(' ', 1)
    if len(command_parts) > 1:
        bot.reply_to(message, f"🔄 Echo: {command_parts[1]}")
    else:
...
if __name__ == "__main__":
    url = f"https://{os.getenv('RENDER_EXTERNAL_HOSTNAME', 'https://bot-lwq9.onrender.com')}/{BOT_TOKEN}"
    bot.remove_webhook()
